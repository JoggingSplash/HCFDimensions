...
final class AABBUtils {
    ...
    public static function replaceBlocks(AxisAlignedBB $box, World $world, ?Block $b = null): void {
        $minX = (int) floor($box->minX);
        $minY = (int) floor($box->minY);
        $minZ = (int) floor($box->minZ);
        $maxX = (int) ceil($box->maxX);
        $maxY = (int) ceil($box->maxY);
        $maxZ = (int) ceil($box->maxZ);
        $block = $b ?? VanillaBlocks::AIR();
    ...
    public static function getNearestStraightPathOutside(AxisAlignedBB $bb, Vector3 $pos): Vector3 {
        if(!$bb->isVectorInside($pos)) return $pos;
        $sideDiffs = [
            "minX" => abs($pos->x - $bb->minX),
            "maxX" => abs($pos->x - $bb->maxX),
            "minZ" => abs($pos->z - $bb->minZ),
            "maxZ" => abs($pos->z - $bb->maxZ),
        ];
        $nearestDist = min($sideDiffs);
        foreach(Facing::HORIZONTAL as $side){ // brute force search :(
            $new = $pos->getSide($side, floor($nearestDist + 2));
            if(!$bb->isVectorInside($new)) return $new;
        }
        return $pos; // todo: properly error out
    }

    public static function getNearestStraightPathInside(AxisAlignedBB $bb, Vector3 $pos): Vector3 {
        if($bb->isVectorInside($pos)) return $pos;
        $sideDiffs = [
            "minX" => abs($pos->x - $bb->minX),
            "maxX" => abs($pos->x - $bb->maxX),
